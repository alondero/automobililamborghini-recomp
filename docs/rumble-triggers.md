...
## The chain (ROM behavior, with native accessory-coexistence overrides)
...
| **Trigger API** `rumble_set(ch, intensity)` | `func_8006A8B4` | `0x80069CB4` | ROM body gates on `0x80110F08[ch]`, clamps to `0x50`, and stores in `0x80110F28[ch]`. The port overrides this leaf with the same clamp/store minus the guest accessory gate, because channel 0 truthfully reports a Controller Pak while native rumble coexists. 5 ROM-wide jals = 5 emitted sites (`BootLoadInitialAssets`, `func_800030F8` ×3, `func_80008ECC`). |
| **Per-frame engine** | `func_8006A910` | `0x80069D10` | ROM body gates every channel on `0x80110F08[ch]` before processing. The port overrides it natively with the verbatim PWM algorithm minus that gate (the same word gates the save flow; one physical accessory makes them exclusive on hardware, the port presents both). Request ≥ `0x47` → motor hard ON; < 6 → stop + `osMotorInit` re-init; else duty-cycle via accumulator `0x80110F38[ch] += intensity³ >> 9 + 4`, motor toggled as it wraps `0x100`. **Analog rumble by pulse-width modulation.** Motor-on state in `0x80110F18[ch]`. 3 ROM-wide jals, all inside `BootLoadInitialAssets`'s per-frame region — the engine ticks every frame. |
| Motor start / stop wrappers | `func_8006A7A0` / `func_8006A82C` | `0x80069BA0` / `0x80069C2C` | ROM bodies replay pre-built Joybus frames only when `0x80110F08[ch]` is set. Native overrides instead forward the ROM's actual requests to SDL and preserve motor state in `0x80110F18[ch]`, without claiming a guest Rumble Pak is inserted. |
...
- For an ares comparison: watchpoint `0x800A3A40` and the frame templates
  ...
  scanning PIF traffic; emulator parity remains unverified without a matching
  capture.
- The old source note that denied motor output is stale. The current source and
  the chain above are the durable explanation.
