...
No live ares comparison is part of the repository's default verification. Do
not claim emulator parity without a capture from the same ROM and version.
...
emulator version, ROM hash, address/units, and result in the relevant issue,
pull request, or subsystem page. Do not copy a conclusion from another port
...
Use this shape in an issue or pull request while an investigation is active.
When the result is settled, move the lasting invariant, test, or limitation to
