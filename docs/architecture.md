...
This page records the stable shape of the current system and its intended
boundaries. It is not a roadmap. New work belongs in an issue and pull request
with a focused test or measurement.
...
See [the patch inventory](../patches/README.md) for the patch-by-patch status.
There is no private RT64 fork in this checkout. Some local patches still mix
generic-looking behavior with Lamborghini-specific policy; do not describe a
local patch as an RT64 feature without checking the pinned source.
...
Evidence should move through this chain:

~~~text
measurement or source observation
        -> issue or pull request while the work is active
        -> named invariant or hypothesis
        -> source/config patch or hook
        -> focused regression test or capture
        -> stable subsystem documentation
~~~

If a step is missing, label the claim as incomplete. Once the work is settled,
the lasting rule belongs in the relevant source, test, or subsystem page.
