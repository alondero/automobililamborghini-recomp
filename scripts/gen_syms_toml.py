...
Current evidence and architectural status belong in the relevant subsystem
page or docs/architecture.md. New comments should use a descriptive label and
a durable local document link.
...
# Issue #3 — widescreen skybox coverage is coordinated by the ROM hooks below and RT64's
# explicit backdrop projection mode. The sky hooks tag each finite panorama projection with
# G_EX_ASPECT_BACKDROP; patch 0011 preserves the world's horizontal FOV while restoring
# independent vertical coverage. The native panorama replacement then supplies the extra
# columns/caps needed for wide viewports. This keeps sky motion tied to the world projection
# without relying on the removed zero-translation heuristic.
...
# only runs in states 7/8 — too late for menu/attract warps). Consumes a Ctrl+Shift+1-6 /
...
# Issue #42 follow-up — widescreen 3P/4P split screen. The race renderer func_800030F8
# sets up each player's 3D view as a 2x2 quadrant viewport (six perspective viewports per
# frame: four half-width quadrants + full-width sky/minimap). Each quadrant covers only half
# the framebuffer width, so under ar_option Expand RT64 squeezes it to the 4:3 centre and the
# composite is pillarboxed (dark side bars). Setting gEXSetViewportAlign(G_EX_ORIGIN_WIDE)
# makes RT64 render a tagged quadrant wide -- filling its output quarter with a Hor+-widened
# FOV (patches/0009); it is a no-op on the already-full-width viewports. The origin is sticky
# global RSP state, so WIDE is set once before the scene's DL calls (0x800044DC, the cursor
# alloc for the first gSPDisplayList of the tail) and reset at the entry of the 2D HUD
# dispatcher func_80050860 (0x8004FC60) -- which func_800030F8 calls after emitting the whole
# 3D scene -- so it spans every quadrant viewport but never reaches the HUD's 2D/ortho draws.
# The natives self-gate on the player count (0x800CE6A4 >= 3): 1P (full screen) and 2P
# (top/bottom, already full width) never set WIDE. Natives in src/lambo_hud_widescreen.c;
# no-op at 4:3. See docs/HUD.md.
[[patches.hook]]
func = "func_800030F8"
before_vram = 0x800044DC
text = "lambo_ws_split_wide_begin(rdram);"

[[patches.hook]]
func = "func_80050860"
before_vram = 0x8004FC60
text = "lambo_ws_split_wide_end(rdram);"

# Issue #84 — 3P/4P split screen skips the sky panorama (the horizon band above the fog is
# the frame-clear fill colour, authored = the near-black variant-2 fog colour). The frame
# dispatcher func_800030F8 selects environment variant 2 for players >= 3 and guards the
# per-viewport sky-emitter call (jal 0x8000F6D8 at 0x80004E9C) with `slti $at, players, 0x3`
# (0x80004E90) + `beq $at, $zero` (0x80004E94). Routing $at through the native lets the
# game's own emitter run for 3P/4P exactly as it does for 1P/2P (it builds its matrices from
# the per-viewport camera angles at 0x800A2EC0/0x800A2EE8, so it is viewport-correct).
# Native in src/lambo_sky_widescreen.cpp; graphics.json key "widescreen_sky_match".
[[patches.hook]]
func = "func_800030F8"
...
# 0x80006094 here. The preceding runtime instruction at 0x80006090 stores the newly
# loaded 0x80251800 context in D_80098238. Applying here keeps the stock downstream
# initialization authoritative and never replaces geometry, navigation, or collision.
[[patches.hook]]
func = "boot_pad_apply_calibration"
before_vram = 0x80006094
text = "{ extern void lambo_track_patch_on_track_loaded(uint8_t*); lambo_track_patch_on_track_loaded(rdram); }"
...
# fixed 20-byte rows with negative holes) and culls each entry against a per-circuit, per-player-count
...
# Stable scene-record identity survives culling/reordering in race and Viewer.
# The linked-list iteration enters at 0x80009AF8 and all exits converge at
# 0x8000E400 before reading record+6 into the next slot. sp+0x1B2 is that slot.
[[patches.hook]]
func = "func_8000A6C0"
before_vram = 0x80009AF8
text = "{ extern void lambo_interpolation_object_begin(uint8_t*, uint32_t); lambo_interpolation_object_begin(rdram, (uint16_t)MEM_H(0x1B2, ctx->r29)); }"

[[patches.hook]]
func = "func_8000A6C0"
before_vram = 0x8000E400
text = "{ extern void lambo_interpolation_object_end(uint8_t*); lambo_interpolation_object_end(rdram); }"
