...
This is a human-owned project. The maintainer decides what is supported, which
trade-offs are acceptable, and whether an architectural or upstream change is
ready. AI-generated suggestions are review material, not project authority.
...
If a dependency change is generic, compare it with the current upstream
project and record a proposal in [docs/upstream-prs.md](docs/upstream-prs.md).
If it only exists to handle Lamborghini data, keep it in the port and explain
...
Long experiments belong in dated investigation pages. Keep source comments
short enough to stay beside the invariant they protect.
