...
[[noreturn]] static void application_exit_success() {
    claim_immediate_exit();
    lambo::ui::save_frontend_preferences();
...
static void vi_cb() {
    lambo::vi::promote_context(g_lambo_rdram);
...
static void update_gfx_stub(void* /*gfx_data*/) {
    std::lock_guard frontend_lock(lambo::ui::frontend_mutex());
    lambo::audio::pump();
    ...
    if (lambo_rt64::enabled()) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            // suppressed. F11 and Alt+Enter were handled above as application shortcuts.
...
static void rumble_apply() {
    ...
    recompinput::set_rumble(0, on != 0 && !lambo::ui::captures_input());
...
static void input_sample() {
    ...
    bool any_player_input = false;

    controller = lambo::ui::sample_frontend_pedals();
    for (int player = 0; player < 4; ++player) {
        uint16_t buttons = 0;
        float x = 0, y = 0;
        const bool connected = recompinput::players::get_player_is_assigned(player);
        if (connected) recompinput::profiles::get_n64_input(player, &buttons, &x, &y);
        const int8_t stick_x = int8_t(std::clamp(x, -1.0f, 1.0f) * N64_STICK_MAX);
        const int8_t stick_y = int8_t(std::clamp(y, -1.0f, 1.0f) * N64_STICK_MAX);
        const uint32_t packed = uint32_t(buttons) | (uint32_t(uint8_t(stick_x)) << 16) | (uint32_t(uint8_t(stick_y)) << 24);
        any_player_input |= packed != 0;
        g_multiplayer_snapshot[player].store(packed, std::memory_order_release);
        g_multiplayer_connected[player].store(connected, std::memory_order_release);
        if (player == 0) { b = buttons; sx = stick_x; sy = stick_y; }
    }

    // Developer shortcuts are not guest mappings; gameplay uses framework profiles above.
    ...
    const uint32_t gated_snap = suppressed_before_publish && (any_player_input ||
...
static bool input_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
    if (controller_num < 0 || controller_num >= 4) return false;
    if (controller_num != 0) {
        if (!g_multiplayer_connected[controller_num].load(std::memory_order_acquire)) return false;
        const uint32_t snap = lambo::input_gate::guest_input_suppressed() ? 0 :
            g_multiplayer_snapshot[controller_num].load(std::memory_order_acquire);
        if (buttons) *buttons = uint16_t(snap);
        if (x) *x = int8_t(snap >> 16) / 127.0f;
        if (y) *y = int8_t(snap >> 24) / 127.0f;
        return true;
    }
...
static ultramodern::input::connected_device_info_t input_device_info(int controller_num) {
    ...
    if (controller_num == 0 || (controller_num > 0 && controller_num < 4 &&
        g_multiplayer_connected[controller_num].load(std::memory_order_acquire))) return { Device::Controller, Pak::None };
